"""
Processed-file bookkeeping for extract_target_data: detecting whether a run's (year, produit,
round) combination already exists, and locating/removing superseded per-run files in overwrite
mode - a theme distinct from org-unit matching, expected-data-structure building, and the
spreadsheet-parsing engine.

Compiling per-run files into one combined dataset is process_target_data's responsibility (see
that pipeline's own run_persistence.py), not this pipeline's.
"""
