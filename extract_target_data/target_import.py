...
def _normalize_products_and_rounds(products, rounds) -> tuple:
    if isinstance(products, str):
        products = [products]
    if isinstance(rounds, (int, np.integer, str)):
        rounds = [rounds]
    round_labels = [f"round {int(r)}" for r in rounds]
    return products, round_labels


def _read_workbook(file) -> pd.DataFrame:
    """Load the first sheet as a headerless grid, failing with an actionable
    message if the file can't be opened or is empty."""
    try:
...
def _detect_layout(raw: pd.DataFrame) -> dict:
    """Discover the header row, geographic level/columns, and column labels -
    everything mechanical the sheet itself can tell us."""
...
def _extract_data_rows(raw: pd.DataFrame, layout: dict) -> pd.DataFrame:
    """Rows below the header, minus blanks (no geo name) and detected totals."""
    data = raw.iloc[layout["header_end"] + 1 :].reset_index(drop=True)
    data = data.dropna(subset=[layout["geo_cols"][0]])
    return drop_aggregate_rows(data, layout["geo_cols"])


def _keep_rows_with_age_values(
    raw: pd.DataFrame, layout: dict, data: pd.DataFrame
) -> pd.DataFrame:
    """Identify the age-bracket columns from their header labels, then keep only
    rows that carry at least one numeric value in one of them. Fails if no age
    column is recognized at all, or if no row survives."""
    ...
    if data.empty:
        fail(
            ...
            f"CAUSE: sous la ligne d'en-tête (ligne {layout['header_end']}), toutes "
            "les lignes ont été écartées car elles étaient vides, sans nom de "
            "district/CSI, sans valeur numérique, ou identifiées comme des totaux.\n"
...
def _build_geo_frame(data: pd.DataFrame, layout: dict) -> pd.DataFrame:
    geo_frame = data[layout["geo_cols"]].copy()
    geo_frame.columns = layout["geo_levels"]
    for c in layout["geo_levels"]:
        geo_frame[c] = geo_frame[c].astype(str).str.strip()
    return geo_frame.reset_index(drop=True)


def _prefer_total_section(candidates: list) -> list:
    ...
    if not any(c["is_total"] for c in candidates):
        return candidates
    ...
    return [c for c in candidates if c["is_total"]]


def _extract_product_pieces(products, candidates, geo_frame, rounding) -> tuple:
    ...
    extracted product would silently under-count the campaign, so it's held back
    (as `incomplete`) rather than partially included."""
    pieces = []
    unknown_products = []
    ...
    return pieces, unknown_products, incomplete


def _fail_on_unknown_products(unknown_products: list) -> None:
    if unknown_products:
        fail(
...
def _fail_on_incomplete_products(
    incomplete: dict, available_ages: list, header_end: int
) -> None:
    if not incomplete:
        return
    details = []
...
def _fail_if_no_pieces(pieces: list, products, available_ages: list) -> None:
    if not pieces:
        fail(
...
def _finalize_tidy_frame(
    pieces: list, geo_levels: list, year: int, round_labels: list
) -> pd.DataFrame:
...
def _log_assumptions_recap() -> None:
...
def import_target_file(
    ...
    """
    products, round_labels = _normalize_products_and_rounds(products, rounds)
    reset_assumptions()

    raw = _read_workbook(file)
    layout = _detect_layout(raw)
    data = _extract_data_rows(raw, layout)
    data = _keep_rows_with_age_values(raw, layout, data)
    geo_frame = _build_geo_frame(data, layout)

    total_cols = detect_total_columns(raw, layout["header_end"])
    ...
    candidates = _prefer_total_section(candidates)

    available_ages = sorted({c["age"] for c in candidates})
    _info(
        f"Niveau géographique: {layout['geo_level']}; en-tête ligne "
        f"{layout['header_end']}; colonnes d'âge détectées: {available_ages}."
    )

    pieces, unknown_products, incomplete = _extract_product_pieces(
        products, candidates, geo_frame, rounding
    )
    _fail_on_unknown_products(unknown_products)
    _fail_on_incomplete_products(incomplete, available_ages, layout["header_end"])
    _fail_if_no_pieces(pieces, products, available_ages)

    tidy = _finalize_tidy_frame(pieces, layout["geo_levels"], year, round_labels)
    _info(f"{len(tidy)} lignes produites (cible totale {int(tidy['cible'].sum())}).")
    _log_assumptions_recap()
    return tidy
