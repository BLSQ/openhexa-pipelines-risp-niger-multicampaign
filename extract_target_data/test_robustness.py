...
def _load_baseline(fn, products, year, rounds, rounding, folder):
    """Load one BASES file and compute its baseline totals.

    Returns ``(raw, base)`` on success. Returns ``(None, None)`` if the file is
    simply missing (silently skipped). Returns ``(None, message)`` if the file
    itself is now rejected: some reference files relied on an approximation (a
    combined-antigen age notation, an absorbed adult bracket, a fixed-ratio
    total split) that the engine now refuses to guess at - until the file's
    headers are corrected, that's expected, and reported plainly rather than
    crashing the whole suite on it.
    """
    ...
    try:
        ...
        print("=" * 88)
        print(f"{fn}\n  BASELINE NOW REJECTED (file needs a header correction): {msg}")
        return None, msg
    print("=" * 88)
    print(f"{fn}\n  baseline: {base}")
    return raw, base
...
def _run_mutations(fn, raw, base, products, year, rounds, rounding, tmp):
    """Apply every MUTATIONS entry to `raw` and check the totals stay == base."""
    passed = failed = 0
    file_failures = []
    for label, mut in MUTATIONS:
        ...
        if ok:
            ...
            failed += 1
            file_failures.append((fn, label, got, base))
    return passed, failed, file_failures


def _run_all_bases(folder, tmp):
    """Baseline + mutation pass over every BASES entry. Returns
    (passed, failed, failures, now_rejected)."""
    passed = failed = 0
    failures = []
    now_rejected = []
    for fn, products, year, rounds, rounding in BASES:
        raw, base_or_msg = _load_baseline(fn, products, year, rounds, rounding, folder)
        if raw is None:
            if base_or_msg is not None:
                now_rejected.append((fn, base_or_msg))
            continue
        p, f, file_failures = _run_mutations(
            fn, raw, base_or_msg, products, year, rounds, rounding, tmp
        )
        passed += p
        failed += f
        failures.extend(file_failures)
    return passed, failed, failures, now_rejected


def _print_mutation_summary(passed, failed, failures, now_rejected):
    print("=" * 88)
...
def _run_error_path_checks(tmp):
    """Cases that SHOULD fail with a clear, actionable TargetImportError."""
    print("\n" + "=" * 88)
...
def _run_minimal_complete_check(tmp):
    """A complete file with the SAME shape must still succeed (guards against
    the completeness check being over-zealous)."""
...
def main():
    ...
    passed, failed, failures, now_rejected = _run_all_bases(folder, tmp)
    _print_mutation_summary(passed, failed, failures, now_rejected)
    _run_error_path_checks(tmp)
    _run_minimal_complete_check(tmp)
