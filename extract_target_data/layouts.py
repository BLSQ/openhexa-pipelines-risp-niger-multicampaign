...
# --------------------------------------------------------------------------- #
# "Type de campagne" parameter (the campaign_name @parameter in pipeline.py). #
#                                                                              #
# The user picks ONE of these French labels; it resolves to BOTH the IASO     #
# campaign_name (choix_campagne - what real IASO submissions carry, used to   #
# look up campaign date ranges) AND the exact products list to import/expect  #
# - products is no longer a separate parameter. Polio is split into coupled/  #
# uncoupled choices (rather than one "coupled or not" choice) specifically so #
# the file's expected product list is always explicit: no product is ever    #
# optional/silently-droppable, so the existing hard-fail-on-incomplete-       #
# product check applies uniformly with no exceptions.                         #
#                                                                              #
# Keep the dict keys in sync with the literal `choices=[...]` list in         #
# pipeline.py's `campaign_name` @parameter (OpenHexa parses choices           #
# statically, so that list can't be built from this dict at decoration time). #
# --------------------------------------------------------------------------- #
CAMPAIGN_CHOICES = {
    ...
    "Polio (non couplée)": ("polio", ["vaccin polio"]),
    "Albendazole et Vitamine A (sans Polio)": ("jnm", ["albendazole", "vitamine A"]),
    "Fièvre jaune": ("fièvre jaune", ["fièvre jaune"]),
    "Rougeole": ("rougeole", ["rougeole"]),
