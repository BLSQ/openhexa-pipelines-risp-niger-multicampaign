...
from config import PRODUCT_STATUS, SEX_TYPE, SITE_TYPE
...
def _make_period_frame(campaign_name: str, produit: str, r, year: int, date_range) -> pd.DataFrame:
...
def _resolve_from_historical_lookup(
    ...
    for r in rounds:
        for produit in products:
            ...
            frames.append(_make_period_frame(campaign_name, produit, r, year, date_range))
...
def build_campaign_period_df(
    ...
    if needs_dates:
        hints = _missing_combo_hints(needs_dates, year, campaign_name, historical_lookup)
...
def _raise_if_unmatched(df: pd.DataFrame, step_label: str) -> pd.DataFrame:
    ...
    if not unmatched.empty:
        ...
        _fail(
            ...
            "de campagne dans config.py) pour ce ou ces produits, puis relancez le "
            "pipeline.",
        )
