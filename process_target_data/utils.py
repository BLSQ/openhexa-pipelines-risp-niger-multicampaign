...
def validate_campaign_filename(filename, valid_campaigns, valid_scales, valid_levels):
    ...
    """
    pattern = (
        r"^Cibles_(?P<camp>[^_]+)_(?P<year>\d{4})_(?P<scale>.+)_(?P<level>[^_]+)\.xlsx$"
    )
    match = re.match(pattern, filename)

    if not match:
        current_run.log_error(
            f"Structure de nommage incorrecte pour le fichier '{filename}'. Format attendu: Cibles_Campagne_Année_Echelle_Niveau.xlsx"
        )
        raise
    ...
    if campaign not in valid_campaigns:
        current_run.log_error(
            f"Nom du fichier '{filename}' contient une campagne non valide: '{campaign}'. Nom de campagnes valides: {', '.join(valid_campaigns)}."
        )
        raise

    if not (2026 <= year <= 2050):
        current_run.log_error(
            f"Nom du fichier '{filename}' contient une année non valide: '{year}'. Années valides: 2026-2050."
        )
        raise

    if aggregation_level not in valid_levels:
        current_run.log_error(
            f"Nom du fichier '{filename}' contient un niveau d'agrégation non valide: '{aggregation_level}'. Niveaux d'agrégation valides: {', '.join(valid_levels)}.",
        )
        raise
    ...
    for s in scales:
        if s.lower() not in valid_scales:
            current_run.log_error(
                f"Nom du fichier '{filename}' contient une échelle de campagne non valide: '{s}'. Échelles valides: {', '.join(valid_scales)}."
            )
            raise
