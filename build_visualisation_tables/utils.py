...
def pyramid_selector(df):
    """
    Selects the row with the maximum 'updated_date' that is not equal to 2023-07-14.
...
def extract_disease_data_for_surveillance_table(
    base_df, disease_name, pfa_col, mapi_mineur_cols, mapi_majeur_cols, group_cols
):
    subset = base_df[group_cols].copy()
    subset["choix_campagne"] = disease_name

    subset["pfa"] = base_df[pfa_col] if pfa_col in base_df.columns else 0

    subset["mapi_mineur"] = 0
    for c in mapi_mineur_cols:
        if c in base_df.columns:
            subset["mapi_mineur"] += base_df[c]

    subset["mapi_majeur"] = 0
    for c in mapi_majeur_cols:
        if c in base_df.columns:
            subset["mapi_majeur"] += base_df[c]
...
# get submission
# foramt submission
# - get choices dict to options(user has to select the columns by seeing the name and returns a dictionary)
# explode the multiple choice case function
# untangle columns like the one of CAR
